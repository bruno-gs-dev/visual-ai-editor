...
var PROVIDER_PRESETS = [
  ...
  {
    id: 'opencode-go',
    name: 'OpenCode Go (assinatura $5/mês)',
    endpoint: 'https://opencode.ai/zen/go/v1/chat/completions',
    models: ['grok-4.5', 'kimi-k2.7-code', 'deepseek-v4-pro', 'qwen3.7-max', 'glm-5.2', 'minimax-m3', 'mimo-v2.5-pro'],
...
/**
 * Fetch locally installed models from a running Ollama instance.
 * @param {string} endpoint - Ollama endpoint (e.g. 'http://localhost:11434/v1/chat/completions')
 * @returns {Promise<{ok: boolean, models: Array<{name: string, size: string}>}>}
 */
function fetchOllamaModels(endpoint) {
  ...
  return fetch(baseUrl + '/api/tags')
    ...
    .then(function (data) {
      if (!data || !data.models) return { ok: false, models: [] };
      var models = data.models.map(function (m) {
        var sizeGB = m.size ? (m.size / (1024 * 1024 * 1024)).toFixed(1) + ' GB' : '';
        var paramSize = m.details && m.details.parameter_size ? m.details.parameter_size : '';
        var label = m.name;
        if (paramSize) label += ' (' + paramSize + ')';
        else if (sizeGB) label += ' (' + sizeGB + ')';
        return { name: m.name, label: label, size: sizeGB, parameterSize: paramSize };
      });
